...
export default function ManualSection() {
  ...
  return (
    <div className="mx-auto max-w-3xl w-full px-6 md:px-12 space-y-10">
      {/* Mode Selector */}
      ...
      <div className="space-y-6 pl-2 sm:pl-10">
        {steps.map((step, i) => {
          const isActive = activeStep === i;
          return (
            <section key={step.index} className="space-y-4">
              {/* Step divider */}
      ...
      {/* Active step reference echo */}
      <div className="text-[9px] font-mono tracking-widest text-zinc-600 dark:text-zinc-500 uppercase">
        CURRENT NODE // {currentStep.index} &middot; {currentStep.title.toUpperCase().replace(/ /g, '_')}
      </div>
    </div>
  );
}
